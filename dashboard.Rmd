...
caption = list(labs(caption =  paste(
  paste("source: drugshortagescanada.ca, last updated:", as_date(file.info(DSC_PATH)$ctime)), 
  "jon.pipitone.ca/research/drug-shortages", 
  sep="\n")))

# load data
if (F) { 
  user = getOption("dsc.username")
  pass = getOption("dsc.password")
  token = dsc_login(user, pass)
  dsc = dsc_search(token)  %>%
    select_if(~ !is.list(.x)) %>%
    write_csv(DSC_PATH)
}

...
emlist = read_csv('data/essential-medicines.csv', trim_ws=T) %>% 
  pivot_longer(-who_class, values_to = "medicine", values_drop_na=T) %>%
  select(-name) %>%
  mutate(drug_name = paste0('\\b', str_to_lower(medicine),'\\b'))

dsc_df %>%
  mutate(drug_name = str_to_lower(en_ingredients)) %>%
  fuzzy_left_join(emlist, by = c("drug_name" = "drug_name"), match_fun = str_detect) %>%
  ...
  #stringdist_inner_join(dsc_df, by = c(medicine = "en_drug_common_name")) %>%
#  filter(str_detect(str_to_lower(en_drug_common_name), str_to_lower(medicine))) %>%
```

...
DSC_CSV = 'data/drugshortagescanada.ca-2019-12-18.csv'
DS_MANUAL_CSV = 'data/drugshortages.ca-manual.csv'
DS_EXPORT_CSV = 'data/drugshortages.ca-export.csv'
ANALYSIS_PERIOD_START = as.Date("2016-01-01")   # start of manual collection period
ANALYSIS_PERIOD_END   = as.Date("2019-12-31")   # end of manual collection period

DPD_YEAR_START = ymd('1980-01-01') # earliest year to look at in the DPD
DPD_YEAR_END = ymd('2020-01-01')   # latest year to look at in the DPD 
...
drugshortagescanada_ca_raw = read_csv(DSC_CSV, col_types = cols(.default = col_character())) %>%
  rename_all(  # Handle updates to the table format
    recode,    # columns listed in: old = new
    "Drug Identification Number" = "DIN", 
    "Actual start date" = "Shortage Start Date", 
    "Actual end date" = "Actual Shortage End Date", 
    "Anticipated start date" = "Anticipated Shortage Start Date", 
    "Estimated end date" = "Estimated Shortage End Date"
  ) %>%
  mutate(
    `Shortage Start Date` = as.Date(`Shortage Start Date`), 
    `Actual Shortage End Date` = as.Date(`Actual Shortage End Date`), 
    `Anticipated Shortage Start Date` = as.Date(`Anticipated Shortage Start Date`), 
    `Estimated Shortage End Date` = as.Date(`Estimated Shortage End Date`)
  )
...
YEAR_START = year(ANALYSIS_PERIOD_START)
YEAR_END = year(ANALYSIS_PERIOD_END)

figure_1_data = drugshortagescanada_ca %>%
...
figure_1 = figure_1_data %>%
  ...
  scale_fill_discrete(guide= FALSE) + # guide_legend(title = "Source")) + 
  scale_y_continuous(expand=c(0,0)) + 
  labs(
    title = "New Drug Shortages per Month, Canada", 
    caption = paste("Source: canadadrugshortages.ca\n", "Last updated:", now()))
...
IG_START_DATE = as.Date('2017-01-01') 
IG_END_DATE = as.Date('2019-12-31')

shortages_with_IG = 
  inner_join(drug_product, drugshortagescanada_ca,  by=c('DRUG_IDENTIFICATION_NUMBER' = 'DIN')) %>%
...
figure_5_data = shortages_with_IG %>%
  ...
  mutate(IG_NAME = INGREDIENT) %>% #paste(INGREDIENT, '(', IG, ')')) %>%
  unique()
...
figure_5 = figure_5_data %>%
  ggplot(aes(x = reorder(IG_NAME, n), y = n, fill = reorder(IG_NAME, n))) +
    ...
    labs(
      title = "Top Shortages by Ingredient, Canada", 
    ...
    scale_fill_discrete(guide = guide_legend(title = "Ingredient")) 
...
MFR_START_YEAR = as.Date('2019-01-01')
MFR_END_YEAR = as.Date('2019-12-31')
...
mfr_shortages = drugshortagescanada_ca %>% 
  filter(between(start_date, MFR_START_YEAR, MFR_END_YEAR)) %>%
...
figure_9 = mfr_percent_shortages %>%
  ggplot(aes(x = percent_products_of_overall, y = percent_shortages_of_overall, label = COMPANY_NAME)) + 
    ...
    labs(title = paste("Top",TOP_N_MFR_MARKETED, "Drugs Producers by Number of Shortages, Canada,", year(MFR_START_YEAR)),
         caption = paste(
            'Source: drug product database, drugshortagescanada.ca\n',
            'Updated:', now(), '\n', 
            'Jon Pipitone, 2020'),
...
ggplot(drug_yearly_status, aes(x = year.date, fill=status)) + 
  ...
  labs(title = "DIN Status by Year",
       ...
       x = "Year", y = "DIN Count")
```
### Number of ATC codes marketed today 

```{r}
drug_yearly_status %>%
  filter(year.date == ymd("2019-01-01"), status == "MARKETED") %>%
  inner_join(dpd_ther) %>%
  group_by(TC_ATC_NUMBER) %>%
  summarize()
```
...
figure_10 = marketed_status %>%
  ggplot(aes(x = year.date, fill = is_innovator)) + 
    ...
    labs(title = "Drug Products (DINs) Available Each Year, Canada, 1970–2017", 
         subtitle = 'Innovator drugs are the first to market in an ingredient group',
         caption = paste('Source: Health Canada Drug Product Database.]nUpdated:', now()), 
         x = "Year", 
         y = "DIN Count") 
...
drugshortagescanada_ca %>%
  ...
  ggplot(aes(x = month, fill = Reason)) + 
    geom_bar() + 
    facet_grid(year ~ . ) + 
    labs(title = "Shortage Reasons", 
         caption = paste('Source: drugshortagescanada.ca.\nUpdated:', now()))
```
